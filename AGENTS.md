...
- **설명:** 전체 서킷 레이아웃을 2D 벡터(SVG)로 렌더링하고 모든 차량의 실시간 트랙 위치, 사고 지점, 3섹터 컬러링을 미니맵에 시각화.
- **아이콘 & 렌더링 요소:**
  ...
  - 사고 발생 지점 (Hazard Beacon): 고휘도 옐로우 점멸 경보 비콘 및 사고 차량 번호 태그.
  - 3섹터 동적 컬러링: 트랙 라인 자체를 S1, S2, S3 구간별 퍼플(세션 최고)/그린(개인 최고)/옐로우(지연)/화이트 펄스(주행 중) 발광.
  - 리벤지 타깃: 사고 유발 차량 위치에 타깃 조준선(`Crosshair`) 오버레이.
- **텔레메트리 변수:** 세션 YAML `WeekendInfo: TrackName/TrackID`, `CarIdxLapDistPct`, `CarIdxTrackSurface`, `CarIdxOnPitRoad`, `SessionFlags`, `SplitTimeInfo: Sectors`.
...
2. **런타임 원격 출처 금지:** 앱 실행 중 서드파티 호스트에서 코드·폰트·이미지를 가져오지 않을 것.
   ...
   **예외 — 테마 공식 서체 설치 (`fetch_theme_font`):** 독점 서체는 §10.3-11에 따라 설치 파일에
   넣을 수 없으므로 사용자가 실행 중에 직접 받는 경로가 불가피합니다. 이 예외는 아래 4개 조건을
   **모두** 만족할 때만 허용하며, 새 원격 경로를 추가할 때도 동일 기준을 적용합니다.
   1. **다이제스트 핀:** 받은 바이트를 `src/services/fonts.ts`의 `sha256`과 대조해 **불일치 시 폐기**.
      바꿔치기된 파일은 등록되지 않습니다(호스트를 신뢰하지 않는 것이 요점).
   2. **호스트 화이트리스트:** Rust `allow_url()`이 핀된 호스트/경로 접두사만 통과시켜,
      침해된 렌더러가 이 커맨드를 범용 fetch 프리미티브로 쓰지 못하게 합니다.
   3. **CSP 불변:** 다운로드는 네이티브가 수행하고 프론트는 `new FontFace(family, bytes)`로 등록하므로
      `connect-src 'self'` / `font-src 'self'`를 **넓히지 않습니다**.
   4. **`public/` 밖 저장:** 받은 파일은 앱 데이터 영역/브라우저 캐시에만 두며 `public/`에 쓰지 않습니다
      (`vite build`가 `public/`을 통째로 릴리스 번들에 복사하므로).
