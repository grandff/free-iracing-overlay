...
export const F1FuelCalculator: Component<Props> = (props) => {
  const current = () => props.fuelLiters !== undefined ? props.fuelLiters : 42.5;
  const maxFuel = () => props.fuelMaxLiters || 110;
  const perLap = () => props.fuelPerLap !== undefined ? props.fuelPerLap : 2.35;
  const laps = () => props.lapsRemaining !== undefined ? props.lapsRemaining : 18.2;
  const needed = () => props.neededToFinish !== undefined ? props.neededToFinish : 28.5;
  ...
  return (
    <div class="relative flex flex-col w-[260px] font-f1 select-none drop-shadow-[0_16px_32px_rgba(0,0,0,0.9)]">
      ...
      <Show when={editPresence.mounted()}>
        ...
        >
          <span class="text-[10px] font-medium text-white/70 whitespace-nowrap">연료 크기</span>
      ...
      <div class="bg-[#12131a]/95 border border-white/15 rounded-lg overflow-hidden shadow-2xl backdrop-blur-xl">
        <div class="flex items-center justify-between bg-[#1a1b24] border-b border-white/10 px-3 py-1.5">
          <div class="flex items-center gap-2 text-white">
            ...
            <span class="text-[10px] font-f1-wide font-extrabold tracking-wider">FUEL CALCULATOR</span>
          </div>
        ...
        <div class="grid grid-cols-2 gap-2 p-2.5 text-white">
          <div class="flex flex-col bg-white/[0.04] p-2 rounded border border-white/[0.06]">
            <span class="text-[9px] font-f1-wide text-white/50">REMAINING</span>
            <span class="text-sm font-f1-num font-bold text-white tnum">{current().toFixed(1)} L</span>
            <span class="text-[9px] text-[#00d26a] font-mono font-semibold">{laps().toFixed(1)} LAPS</span>
          </div>

          <div class="flex flex-col bg-white/[0.04] p-2 rounded border border-white/[0.06]">
            <span class="text-[9px] font-f1-wide text-white/50">PER LAP / PIT REQ</span>
            <span class="text-sm font-f1-num font-bold text-amber-300 tnum">{perLap().toFixed(2)} L/L</span>
