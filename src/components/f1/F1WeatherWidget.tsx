...
export const F1WeatherWidget: Component<Props> = (props) => {
  ...
  return (
    <div class="relative flex flex-col font-f1 select-none drop-shadow-[0_16px_32px_rgba(0,0,0,0.9)]">
      ...
      <Show when={editPresence.mounted()}>
        ...
        >
          <span class="text-[10px] font-medium text-white/70 whitespace-nowrap">날씨 크기</span>
      ...
      {/* Main Weather Pill */}
      <div class="flex items-center h-9 bg-[#12131a]/95 border border-white/15 rounded-lg overflow-hidden shadow-2xl backdrop-blur-xl px-3 gap-3 text-white text-xs">
        <div class="flex items-center gap-1.5 text-white/80">
          <IconCompass size={14} class="text-cyan-400" />
          <span class="font-mono text-[11px] tnum">{wind()} km/h</span>
        </div>
        ...
        <div class="flex items-center gap-1 text-[11px] font-mono">
          ...
          <span class="font-bold text-white tnum">{air().toFixed(1)}°C</span>
        </div>
        ...
        <div class="flex items-center gap-1 text-[11px] font-mono">
          ...
          <span class="font-bold text-amber-400 tnum">{track().toFixed(1)}°C</span>
        </div>
        ...
        <div class="flex items-center gap-1 text-[11px] font-mono">
          ...
          <span class="font-bold text-[#00d26a] tnum">{rain()}%</span>
        </div>
      </div>
    </div>
  );
