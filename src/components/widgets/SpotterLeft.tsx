...
export const SpotterLeft: Component<Props> = (props) => {
  ...
  return (
    <div class="relative flex items-center font-sans select-none pointer-events-auto">
      <Show when={editPresence.mounted()}>
        <div
          class={`absolute -top-10 left-0 z-50 flex items-center gap-2 px-2.5 py-1 bg-[#1c1c24]/95 border border-white/15 rounded-full shadow-lg text-white pointer-events-auto whitespace-nowrap apple-pill-enter ${
            editPresence.visible() ? "is-visible" : "is-hidden"
          }`}
        >
          <span class="text-[10px] font-medium text-white/70">{t().wSpotterLeft}</span>
      ...
      <div
        class={`flex items-center gap-2.5 px-3 py-2.5 rounded-r-xl border-y border-r transition-all duration-150 ${
          isDanger()
      ...
      >
        <div class="flex flex-col items-center justify-center w-5">
          ...
          <span class="text-[8px] font-mono font-black uppercase mt-0.5 tracking-wider">LEFT</span>
        </div>
