...
export const LapDelta: Component<Props> = (props) => {
  const delta = () => (props.deltaSeconds !== undefined ? props.deltaSeconds : props.delta !== undefined ? props.delta : -0.234);
  const isFaster = () => delta() <= 0;
  const isPurple = () => delta() <= -0.5;
  ...
  return (
    <div class="relative flex flex-col font-sans select-none shadow-2xl">
      <Show when={editPresence.mounted()}>
        ...
        >
          <span class="text-[10px] font-medium text-white/70 whitespace-nowrap">{t().wLapDelta}</span>
      ...
      <div class="flex items-center h-9 bg-[#12131a]/95 border border-white/15 rounded-lg overflow-hidden shadow-2xl">
        <div class="flex items-center gap-1.5 px-3 bg-[#1c1c26] h-full border-r border-white/10 text-white/70">
          ...
          <span class="text-[10px] font-mono font-extrabold tracking-wider">DELTA</span>
        </div>
        ...
        >
          ...
          <span class="font-mono font-bold text-sm tracking-wider tabular-nums">
            {deltaText()}s
          </span>
        </div>

        <div class="px-2.5 text-[9px] font-mono text-white/50 tracking-wider">
